...
export default function FormLayout({ heading, para, formData, icons, selectField }) {
    return (
        <Form xs={10} className='main-form-page'>
           <h1 className='text-center pt-4 fw-bold' style={{fontSize:"35px",color:"#FE5A0E"}}>{heading}</h1>
            <p className='text-center pt-3 form-para' style={{color:"#3C3C3C" ,fontSize:"15px",fontWeight:"500"}}>{para}</p>
            {formData.map((field, index) => (
                <FormGroup key={index} className='d-flex justify-content-center align-items-center py-3'>
                    {field.type !== 'select' && (
